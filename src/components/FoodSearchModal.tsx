...
import { searchFoods, lookupBarcode, createFood } from '../api/foods'
...
type Mode = 'pick' | 'portion' | 'create' | 'scan'
...
export default function FoodSearchModal({ mealLabel = 'Meal', onAdd, onClose }: Props) {
  // Slide-in animation: mount with open=false, flip to true after one frame
  ...
  const handleCreate = async () => {
    ...
    let cal100g: number
    let pro100g: number | null = null
    let carb100g: number | null = null
    let fat100g: number | null = null
    let fib100g: number | null = null
    let servSizeG: number | null = null
    ...
    try {
      const food = await createFood({
        ...
        proteinPer100g: pro100g,
        carbsPer100g: carb100g,
        fatPer100g: fat100g,
        fiberPer100g: fib100g,
        servingSizeG: servSizeG,
      })
      // Log with the serving size if defined, else 100g
  ...
  const goBack = () => { setMode('pick'); setSelected(null); setScanError(null) }
        ...
        {mode === 'pick' && (
          <>
            ...
            <div className="ct-pick-scroll">
              ...
              {!q.trim() && <div className="ct-pick-label">Type to search foods</div>}
            ...
            <div className="ct-form">
              ...
              <div className="ct-field">
                ...
                <div className="ct-unit-seg" style={{ alignSelf: 'flex-start' }}>
                  <button type="button"
                    className={'ct-unit-btn' + (measureMode === 'per100g' ? ' active' : '')}
                    onClick={() => setMeasureMode('per100g')}
                    style={{ padding: '0 14px' }}>
                    Per 100g
                  </button>
                  <button type="button"
                    className={'ct-unit-btn' + (measureMode === 'perServing' ? ' active' : '')}
                    onClick={() => setMeasureMode('perServing')}
                    style={{ padding: '0 14px' }}>
                    Per serving
                  </button>
                </div>
                ...
                <div className="ct-field">
                  ...
                  <div className="ct-portion-row">
                    <input className="ct-in" type="number" inputMode="decimal" min="0.1" step="0.1"
                      ...
                      onChange={(e) => setForm((f) => ({ ...f, servingQty: e.target.value }))}
                      style={{ flex: 1 }} />
                    <div className="ct-unit-seg">
                      {(['g', 'oz'] as const).map((u) => (
                        <button key={u} type="button"
                          className={'ct-unit-btn' + (form.servingUnit === u ? ' active' : '')}
                          onClick={() => setForm((f) => ({ ...f, servingUnit: u }))}>
                          {u}
                        </button>
                      ))}
                    </div>
                  </div>
                  {form.servingUnit === 'oz' && servingGrams() > 0 && (
                    <span style={{ fontFamily: 'var(--font-display)', fontSize: 10, color: 'var(--muted)', marginTop: 4 }}>
                      {Math.round(servingGrams() * 10) / 10}g
                    </span>
                  )}
                </div>
              )}
